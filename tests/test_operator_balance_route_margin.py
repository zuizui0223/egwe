...
def test_full_feedback_margin_equals_qonly_plus_exact_recoupling_shift() -> None:
    theta = 0.58
    for q, t, g, d in [(0.4, 0.9, 0.8, 0.8), (0.8, 0.2, 0.3, 1.0), (0.6, 0.6, 0.6, 0.5)]:
        ...
        shift = recoupling_shift(q, t, g, d)
        assert math.isclose(full, base + shift, abs_tol=1e-12)


def test_critical_bundle_is_exact_recoupling_boundary() -> None:
    theta = 0.60
    q = 0.65
    density = 0.95
...
def test_upward_recoupling_can_move_qonly_below_switch_above_switch() -> None:
    theta = 0.60
    q = 0.65
    density = 0.95
    ...
    assert full > 0
    assert recoupling_shift(q, bundle, bundle, density) > -base


def test_recoupling_can_also_suppress_when_bundle_is_below_q() -> None:
    theta = 0.50
    q = 0.8
    density = 1.0
    base = q_only_route_margin(q, density, theta)
    ...
    full = route_margin(q, bundle, bundle, density, theta)
    assert full < 0
    assert recoupling_shift(q, bundle, bundle, density) < 0
